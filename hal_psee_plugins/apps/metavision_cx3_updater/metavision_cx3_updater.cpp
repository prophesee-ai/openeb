...
int main(int argc, char *argv[]) {
    ...
    std::vector<libusb_device_handle *> handles; // vector of device handles
    // listing of usb devices for cx3, (name, vid, pid)
    const std::unordered_map<std::string, std::pair<uint16_t, uint16_t>> USB_DEVICES = {
        {"EVK3", std::make_pair(0x04b4, 0x00f4)}, {"EVK4", std::make_pair(0x04b4, 0x00f5)}};
    ...
    if (r <= 0) {
        MV_LOG_ERROR() << "Cannot fetch system ID" << libusb_error_name(r);
        return 1;
    } else {

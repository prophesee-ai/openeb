...
Gen41EventTrailFilterModule::Gen41EventTrailFilterModule(const std::shared_ptr<RegisterMap> &register_map,
                                                         const std::string &sensor_prefix) :
    register_map_(register_map), sensor_prefix_(sensor_prefix) {}

std::set<I_EventTrailFilterModule::Type> Gen41EventTrailFilterModule::get_available_types() const {
    return {I_EventTrailFilterModule::Type::STC_CUT_TRAIL, I_EventTrailFilterModule::Type::TRAIL};
}

bool Gen41EventTrailFilterModule::set_threshold(uint32_t threshold) {
...
bool Gen41EventTrailFilterModule::set_type(I_EventTrailFilterModule::Type type) {
...
bool Gen41EventTrailFilterModule::enable(bool state) {
    // Bypass filter
    ...
    if (filtering_type_ == I_EventTrailFilterModule::Type::STC_CUT_TRAIL) {
        (*register_map_)[sensor_prefix_ + "stc/stc_param"].write_value(
            {{"stc_enable", 1}, {"stc_threshold", threshold_ms_ * 1000}});
...
bool Gen41EventTrailFilterModule::is_enabled() const {
    return enabled_;
}

I_EventTrailFilterModule::Type Gen41EventTrailFilterModule::get_type() const {
    return filtering_type_;
}

uint32_t Gen41EventTrailFilterModule::get_threshold() const {
    return threshold_ms_ * 1000;
}

uint32_t Gen41EventTrailFilterModule::get_max_supported_threshold() const {
    return 100000;
}

uint32_t Gen41EventTrailFilterModule::get_min_supported_threshold() const {
    return 1000;
}

...
void initialize_plugin(void *plugin_ptr) {
    ...
    auto &evk4_disc = plugin.add_camera_discovery(std::make_unique<TzCameraDiscovery>());
    evk4_disc.factory().insert("treuzell,streamer", TzStreamer::build);
    evk4_disc.factory().insert("psee,ccam5_gen42", TzImx636::build, TzImx636::can_build);
    evk4_disc.factory().insert("ti,tmp103", TiTmp103::build);
#endif

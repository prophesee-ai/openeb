...
#include "decoders/base/base_event_types.h"

namespace Metavision {

constexpr uint8_t EVT2EventsTimeStampBits = 6;

// LEFT_APS_START and LEFT_APS_END are inverted compared to their definition because we
// explicitly inverse EM polarity in the driver when we start a gen3 EM camera.
// RIGHT_APS_END and RIGHT_APS_START are untouched as we don't have sysleyEM stereo system connected by FPGA.

enum class EVT2EventTypes : EventTypesUnderlying_t {
    LEFT_TD_LOW = static_cast<EventTypesUnderlying_t>(
        BaseEventTypes::LEFT_TD_LOW), // Left camera TD event, decrease in illumination (polarity '0')
    LEFT_TD_HIGH = static_cast<EventTypesUnderlying_t>(
        BaseEventTypes::LEFT_TD_HIGH), // Left camera TD event, increase in illumination (polarity '1')
    LEFT_APS_START = static_cast<EventTypesUnderlying_t>(
        BaseEventTypes::LEFT_APS_END), // Left camera APS event, measurement start (polarity '0')
    LEFT_APS_END = static_cast<EventTypesUnderlying_t>(
        BaseEventTypes::LEFT_APS_START), // Left camera APS event, measurement end (polarity '1')
    RIGHT_TD_LOW = static_cast<EventTypesUnderlying_t>(
        BaseEventTypes::RIGHT_TD_LOW), // Right camera TD event, decrease in illumination (polarity '0')
    RIGHT_TD_HIGH = static_cast<EventTypesUnderlying_t>(
        BaseEventTypes::RIGHT_TD_HIGH), // Right camera TD event, increase in illumination (polarity '1')
    RIGHT_APS_END = static_cast<EventTypesUnderlying_t>(
        BaseEventTypes::RIGHT_APS_END), // Right camera APS event, measurement end (polarity '0')
    RIGHT_APS_START = static_cast<EventTypesUnderlying_t>(
        BaseEventTypes::RIGHT_APS_START), // Right camera APS event, measurement start (polarity '1')
    ...
    STEREO_DISP = static_cast<EventTypesUnderlying_t>(BaseEventTypes::STEREO_DISP), // Stereo disparity event

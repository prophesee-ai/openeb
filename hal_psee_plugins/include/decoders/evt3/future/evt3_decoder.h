...
#ifndef METAVISION_HAL_FUTURE_EVT3_DECODER_H
#define METAVISION_HAL_FUTURE_EVT3_DECODER_H
...
private:
    ...
    bool reset_timestamp_impl(const timestamp &t) override {
        ...
        // !TODO : fix this
        // reset internal state, this is not enough right now, as the state will be used as is
        // when decoding next events -> we should reset the state from the next events instead
...
namespace {
void log_protocol_violation(const DecoderProtocolViolation &protocol_violation_type) {
...
inline std::unique_ptr<I_Decoder> make_evt3_decoder(
    ...
    if (std::getenv("MV_FLAGS_EVT3_THROW_ON_NON_MONOTONIC_TIME_HIGH") || std::getenv("MV_FLAGS_EVT3_ROBUST_DECODER")) {
        MV_HAL_LOG_INFO() << "Use EVT3 Robust decoder.";
        ...
        MV_HAL_LOG_INFO() << "Use EVT3 Unsafe decoder.";
        decoder = std::make_unique<UnsafeEVT3Decoder>(time_shifting_enabled, height, width, event_cd_decoder,
                                                      event_ext_trigger_decoder);
    }

    decoder->add_protocol_violation_callback(log_protocol_violation);
...
#endif /* METAVISION_HAL_FUTURE_EVT3_DECODER_H */

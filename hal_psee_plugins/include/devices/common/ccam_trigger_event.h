...
#ifndef METAVISION_HAL_CCAM_TRIGGER_EVENT_H
#define METAVISION_HAL_CCAM_TRIGGER_EVENT_H
...
class CCamTriggerEvent : public PseeTriggerIn {
public:
    /// @brief Constructor
    CCamTriggerEvent(const std::shared_ptr<RegisterMap> &register_map,
                     const std::shared_ptr<PseeDeviceControl> &device_control, const std::string &prefix);

    enum class Channels { TRIG_IN = 1, TRIG_OUT = 6, AUX_TRIG_IN = 7 };
...
private:
    ...
    std::string prefix_;
    const std::vector<Channels> chan_ids_{Channels::TRIG_IN, Channels::TRIG_OUT, Channels::AUX_TRIG_IN};
};

} // namespace Metavision

#endif // METAVISION_HAL_CCAM_TRIGGER_EVENT_H

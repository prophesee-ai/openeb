...
#ifndef METAVISION_HAL_FUTURE_FILE_DATA_TRANSFER_H
#define METAVISION_HAL_FUTURE_FILE_DATA_TRANSFER_H
...
private:
    FileDataTransfer(const std::shared_ptr<std::istream> &stream, uint32_t raw_event_size_bytes,
                     const RawFileConfig &config);

    void start_impl(BufferPtr buffer) override;
    ...
    std::shared_ptr<std::istream>
        stream_to_read_; // stored as shared_ptr as a workaround, unique_ptr should preferred here
};

} // namespace Future
} // namespace Metavision

#endif // METAVISION_HAL_FUTURE_FILE_DATA_TRANSFER_H

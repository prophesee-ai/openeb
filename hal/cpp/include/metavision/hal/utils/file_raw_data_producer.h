...
#ifndef METAVISION_HAL_RAW_DATA_PRODUCER_H
#define METAVISION_HAL_RAW_DATA_PRODUCER_H
...
class FileRawDataProducer : public DataTransfer::RawDataProducer {
public:
    ...
    /// @param buffer_pool The bufferpool to be used to allocate memory for the data transfer
    FileRawDataProducer(std::unique_ptr<std::istream> stream, uint32_t raw_event_size_bytes,
                        const RawFileConfig &config,
                        DataTransfer::DefaultBufferPool buffer_pool = DataTransfer::DefaultBufferPool::make_bounded(4));
...
private:
    ...
    using DefaultBufferPtr = DataTransfer::DefaultBufferPool::ptr_type;
    ...
    DefaultBufferPtr seek_buffer_; // extra slack buffer we can use to unblock any pending acquire from the transfer
                                   // buffer pool before doing a seek
...
#endif // METAVISION_HAL_FILE_RAW_DATA_PRODUCER_H

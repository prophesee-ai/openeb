...
#ifndef METAVISION_SDK_DRIVER_FRAME_HISTO_H
#define METAVISION_SDK_DRIVER_FRAME_HISTO_H

#include <memory>
#include <functional>

// Metavision SDK Base Histogram
...
/// @brief Type alias for a callback on a @ref RawEventFrameHisto
using RawEventFrameHistoCallback = std::function<void(const RawEventFrameHisto &h)>;

/// @brief Facility class to handle RawEventFrameHisto
class FrameHisto {
public:
    /// @brief Destructor
    ///
    ...
    /// @brief Subscribes to Histogram events
    ///
    /// Registers a callback that will be called each time a RawEventFrameHisto has been decoded.
    ///
    /// @param cb Callback to call each time a RawEventFrameHisto has been decoded
    /// @sa @ref RawEventFrameHistoCallback
...
#endif // METAVISION_SDK_DRIVER_FRAME_HISTO_H

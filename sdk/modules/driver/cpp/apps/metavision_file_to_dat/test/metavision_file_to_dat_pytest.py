...
TRIGGER_P_MASK = 2**4 - 1  # 28 zeros followed by 4 ones when formulated as a binary number.
TRIGGER_ID_MASK = 2**32 - 2**26  # 6 ones followed by 26 zeros.


def run_file_to_dat_on_recording_and_check_result(
        filename_full,
        width_expected,
        height_expected,
        number_cd_expected,
    ...
    expected_generated_cd_file = input_file.replace(".raw", "_cd.dat").replace(".hdf5", "_cd.dat")
    # Just to be sure, check that the DAT file does not already exist, otherwise the test could be misleading
    assert not os.path.exists(expected_generated_cd_file)
    ...
    assert os.path.exists(expected_generated_cd_file)

    # Now open the file and check for its contents
    with open(expected_generated_cd_file, 'rb') as f:
        # Parse header
        width = -1
        height = -1
        begin_events_pos = 0
    ...
    # Check DAT file has been written
    expected_generated_trigger_file = input_file.replace(".raw", "_trigger.dat").replace(".hdf5", "_trigger.dat")
    assert os.path.exists(expected_generated_trigger_file)

    # Now open the file and check for its contents
    with open(expected_generated_trigger_file, 'rb') as f:
        # Parse header
        width = -1
        height = -1
        begin_events_pos = 0
        ...
        # Assert event type and size
        assert ev_type == 14  # Trigger
        assert ev_size == 8

        # Now check total number of CD events and time of first and last
        data = np.fromfile(f, dtype=[('t', 'u4'), ('p_id', 'i4')])

        p = np.bitwise_and(data["p_id"], TRIGGER_P_MASK)
        id = np.right_shift(np.bitwise_and(data["p_id"], TRIGGER_ID_MASK), 26)

        nr_ext_triggers = len(data)
        assert nr_ext_triggers == number_trigger_expected

        # Check first 10 events :
        for idx in range(0, 10):
            ev = {'p': p[idx], 'id': id[idx], 't': data["t"][idx]}
            assert ev == first_10_ext_trigger_events_expected[idx], "Error on event nr {}".format(idx)

        # Check the 10 events in the middle:
        idx_ev = nr_ext_triggers // 2 - 5
        for idx in range(0, 10):
            ev = {'p': p[idx_ev], 'id': id[idx_ev], 't': data["t"][idx_ev]}
...
def pytestcase_test_metavision_file_to_dat_show_help():
    """
    Checks output of metavision_file_to_dat when displaying help message
    """

    cmd = "./metavision_file_to_dat --help"
    output, error_code = pytest_tools.run_cmd_setting_mv_log_file(cmd)
...
def pytestcase_test_metavision_file_to_dat_non_existing_input_file():
    """
    Checks that metavision_file_to_dat returns an error when passing an input file that doesn't exist
    """
    ...
    cmd = "./metavision_file_to_dat -i {}".format(input_rawfile)
    output, error_code = pytest_tools.run_cmd_setting_mv_log_file(cmd)
...
def pytestcase_test_metavision_file_to_dat_missing_input_args():
    """
    Checks that metavision_file_to_dat returns an error when not passing required input args
    """

    cmd = "./metavision_file_to_dat"
    output, error_code = pytest_tools.run_cmd_setting_mv_log_file(cmd)
...
def pytestcase_test_metavision_file_to_dat_on_gen31_raw_recording(dataset_dir):
    """
    Checks result of metavision_file_to_dat application on dataset gen31_timer.raw
    """
    ...
    run_file_to_dat_on_recording_and_check_result(
...
def pytestcase_test_metavision_file_to_dat_on_gen4_evt2_raw_recording(dataset_dir):
    """
    Checks result of metavision_file_to_dat application on dataset gen4_evt2_hand.raw
    """
...
def pytestcase_test_metavision_file_to_dat_on_gen4_evt3_raw_recording(dataset_dir):
    """
    Checks result of metavision_file_to_dat application on dataset gen4_evt3_hand.raw
    """
...
def pytestcase_test_metavision_file_to_dat_on_gen4_evt3_with_triggers_raw_recording(dataset_dir):
    """
    Checks result of metavision_file_to_dat application on dataset blinking_gen4_with_ext_triggers.raw
    """
...
def pytestcase_test_metavision_file_to_dat_on_gen31_hdf5_recording(dataset_dir):
    """
    Checks result of metavision_file_to_dat application on dataset gen31_timer.hdf5
    """
...
@pytest.mark.skipif("HAS_HDF5" not in os.environ or os.environ["HAS_HDF5"] != "TRUE", reason="hdf5 not available")
def pytestcase_test_metavision_file_to_dat_on_gen4_evt2_hdf5_recording(dataset_dir):
    """
    Checks result of metavision_file_to_dat application on dataset gen4_evt2_hand.hdf5
    """
...
@pytest.mark.skipif("HAS_HDF5" not in os.environ or os.environ["HAS_HDF5"] != "TRUE", reason="hdf5 not available")
def pytestcase_test_metavision_file_to_dat_on_gen4_evt3_hdf5_recording(dataset_dir):
    """
    Checks result of metavision_file_to_dat application on dataset gen4_evt3_hand.hdf5
    """
...
@pytest.mark.skipif("HAS_HDF5" not in os.environ or os.environ["HAS_HDF5"] != "TRUE", reason="hdf5 not available")
def pytestcase_test_metavision_file_to_dat_on_gen4_evt3_with_triggers_hdf5_recording(dataset_dir):
    """
    Checks result of metavision_file_to_dat application on dataset blinking_gen4_with_ext_triggers.hdf5
    """

...
#include "metavision/sdk/driver/internal/event_file_writer_internal.h"
#include "metavision/sdk/driver/raw_event_file_writer.h"

namespace Metavision {

class RAWEventFileWriter::Private {
public:
    Private(RAWEventFileWriter &writer, const std::string &path,
...
RAWEventFileWriter::RAWEventFileWriter(const std::string &path,
                                         const std::unordered_map<std::string, std::string> &metadata_map) :
    EventFileWriter(path), pimpl_(new Private(*this, path, metadata_map)) {}

RAWEventFileWriter::~RAWEventFileWriter() {
    close();
}

void RAWEventFileWriter::open_impl(const std::string &path) {
    pimpl_->open_impl(path);
}

void RAWEventFileWriter::close_impl() {
    pimpl_->close_impl();
}

bool RAWEventFileWriter::is_open_impl() const {
    return pimpl_->is_open_impl();
}

void RAWEventFileWriter::add_metadata_impl(const std::string &key, const std::string &value) {
    pimpl_->add_metadata_impl(key, value);
}

void RAWEventFileWriter::add_metadata_map_from_camera_impl(const Camera &camera) {
    pimpl_->add_metadata_map_from_camera_impl(camera);
}

void RAWEventFileWriter::remove_metadata_impl(const std::string &key) {
    pimpl_->remove_metadata_impl(key);
}

bool RAWEventFileWriter::add_events_impl(const EventCD *begin, const EventCD *end) {
    throw std::runtime_error("RAW does not support writing decoded events");
}

bool RAWEventFileWriter::add_events_impl(const EventExtTrigger *begin, const EventExtTrigger *end) {
    throw std::runtime_error("RAW does not support writing decoded events");
}

bool RAWEventFileWriter::add_raw_data(const std::uint8_t *ptr, size_t size) {
    return pimpl_->add_raw_data(ptr, size);
}

void RAWEventFileWriter::flush_impl() {

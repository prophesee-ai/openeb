...
#include "metavision/sdk/driver/internal/event_file_reader_internal.h"
...
public:
    Private(DATEventFileReader &reader) : reader_(reader), data_stream_(reader.get_path()) {
        ...
        metadata_["system_ID"]  = "0";

        if (boost::filesystem::exists(reader.get_path())) {
            setup_dat_stream(reader.get_path());
        } else {
            bool found_target_files = false;

            if (boost::filesystem::exists(reader.get_path() + "_cd.dat")) {
                setup_dat_stream(reader.get_path() + "_cd.dat");
                found_target_files = true;
            }
            if (boost::filesystem::exists(reader.get_path() + "_td.dat")) {
                setup_dat_stream(reader.get_path() + "_td.dat");
                found_target_files = true;
            }
            if (boost::filesystem::exists(reader.get_path() + "_trigger.dat")) {
                setup_dat_stream(reader.get_path() + "_trigger.dat");
                found_target_files = true;
            }

            if (!found_target_files) {
                throw std::runtime_error("Could not find dat files for \"" + reader.get_path() + "\"");
            }
        }
    }
...
private:
    ...
    class DATStream {
    public:
        DATStream(DATEventFileReader &reader, std::ifstream &&ifs, const std::string &path) :
    ...
    void setup_dat_stream(const std::string &path) {
...
DATEventFileReader::DATEventFileReader(const std::string &path) :

...
EventFileWriter::Private::Private(EventFileWriter &writer, const std::string &path) :
...
void EventFileWriter::Private::open(const std::string &path) {
...
const std::string &EventFileWriter::Private::get_path() const {
    return path_;
}
...
bool EventFileWriter::Private::add_events(const EventCD *begin, const EventCD *end) {
    ...
    if (cd_buffer_ptr_->size() > kMaxEventCDBufferSize) {
...
bool EventFileWriter::Private::add_events(const EventExtTrigger *begin, const EventExtTrigger *end) {
    ...
    if (ext_trigger_buffer_ptr_->size() > kMaxEventTriggerBufferSize) {
...
EventFileWriter::EventFileWriter(const std::string &path) : pimpl_(new Private(*this, path)) {}

EventFileWriter::~EventFileWriter() {}

void EventFileWriter::open(const std::string &path) {
    pimpl_->open(path);
}
...
const std::string &EventFileWriter::get_path() const {

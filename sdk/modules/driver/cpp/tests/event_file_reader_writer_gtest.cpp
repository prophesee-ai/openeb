...
#include "metavision/sdk/driver/raw_event_file_reader.h"
...
TEST_F_WITH_DATASET(HDF5EventFileReader_Gtest, constructor_valid) {
    std::string dataset_file_path =
        (boost::filesystem::path(GtestsParameters::instance().dataset_dir) / "openeb" / "gen31_timer.hdf5").string();
    std::unique_ptr<HDF5EventFileReader> reader;
...
TEST_F_WITH_DATASET(HDF5EventFileReader_Gtest, get_metadata_map) {
    ...
    it = metadata_map.find("system_ID");
    ASSERT_TRUE(it != metadata_map.end());
    EXPECT_EQ("28", it->second);
...
TEST_F(HDF5EventFileReader_Gtest, read_cd_events_written_without_direct_calls) {
...

    {
        ...
        // and the ECF plugin
        auto file = H5::H5File(tmp_file_, H5F_ACC_TRUNC);
        file.createGroup("/CD");
...
TEST_WITH_DATASET(DATEventFileReader_Gtest, constructor_valid) {
    std::string dataset_file_path = (boost::filesystem::path(GtestsParameters::instance().dataset_dir) / "openeb" /
                                     "core" / "event_io" / "recording_td.dat")
                                        .string();
...
TEST_WITH_DATASET(DATEventFileReader_Gtest, read_delayed_recording) {
    std::string dataset_file_path =
        (boost::filesystem::path(GtestsParameters::instance().dataset_dir) / "openeb" / "test_start_after_0.dat")
            .string();
    DATEventFileReader reader(dataset_file_path);
    size_t num_cd_events = 0;
    reader.add_read_callback(
...
class RAWEventFileWriter_Gtest : public GTestWithTmpDir {
protected:
    virtual void SetUp() {
...
TEST_F(RAWEventFileWriter_Gtest, empty_constructor) {
    std::unique_ptr<RAWEventFileWriter> writer;
    ASSERT_NO_THROW(writer = std::make_unique<RAWEventFileWriter>());
}

TEST_F(RAWEventFileWriter_Gtest, constructor_invalid) {
    std::unique_ptr<RAWEventFileWriter> writer;
    std::string file =
        (boost::filesystem::path(tmpdir_handler_->get_full_path("inexistent_directory")) / "file.raw").string();
    ASSERT_THROW(writer = std::make_unique<RAWEventFileWriter>(file), std::runtime_error);
}

TEST_F(RAWEventFileWriter_Gtest, constructor) {
    {
        std::unique_ptr<RAWEventFileWriter> writer;
        ASSERT_NO_THROW(writer = std::make_unique<RAWEventFileWriter>(tmp_file_));
    }
    {
...
TEST_F(RAWEventFileWriter_Gtest, not_is_open) {
    std::unique_ptr<RAWEventFileWriter> writer;
    writer = std::make_unique<RAWEventFileWriter>();
    ASSERT_FALSE(writer->is_open());
}

TEST_F(RAWEventFileWriter_Gtest, is_open) {
    std::unique_ptr<RAWEventFileWriter> writer;
    writer = std::make_unique<RAWEventFileWriter>(tmp_file_);
    ASSERT_TRUE(writer->is_open());
}

TEST_F(RAWEventFileWriter_Gtest, open) {
    std::unique_ptr<RAWEventFileWriter> writer;
    ASSERT_NO_THROW(writer = std::make_unique<RAWEventFileWriter>());
    ASSERT_NO_THROW(writer->open(tmp_file_));
}

TEST_F(RAWEventFileWriter_Gtest, close) {
    std::unique_ptr<RAWEventFileWriter> writer;
    writer = std::make_unique<RAWEventFileWriter>(tmp_file_);
    ASSERT_NO_THROW(writer->close());
}

TEST_F(RAWEventFileWriter_Gtest, open_close) {
    std::unique_ptr<RAWEventFileWriter> writer;
    ASSERT_NO_THROW(writer = std::make_unique<RAWEventFileWriter>());
    ASSERT_NO_THROW(writer->open(tmp_file_));
    ASSERT_TRUE(writer->is_open());
...
TEST_F_WITH_DATASET(RAWEventFileWriter_Gtest, constructor_metadata_map) {
    {
        std::string dataset_file_path =
            (boost::filesystem::path(GtestsParameters::instance().dataset_dir) / "openeb" / "gen31_timer.raw").string();
        ...
        std::unique_ptr<RAWEventFileWriter> writer;
        ASSERT_NO_THROW(writer = std::make_unique<RAWEventFileWriter>(tmp_file_, m));
    }
    {
        std::ifstream ifs(tmp_file_);
        GenericHeader header(ifs);
...
TEST_F(RAWEventFileWriter_Gtest, get_path) {
    RAWEventFileWriter writer(tmp_file_);
    ASSERT_EQ(tmp_file_, writer.get_path());
}

TEST_F_WITH_DATASET(RAWEventFileWriter_Gtest, add_metadata) {
    {
        std::string dataset_file_path =
            (boost::filesystem::path(GtestsParameters::instance().dataset_dir) / "openeb" / "gen31_timer.raw").string();
        std::ifstream ifs(dataset_file_path);
...
TEST_F_WITH_DATASET(RAWEventFileWriter_Gtest, add_metadata_no_flush_succeed) {
    std::string dataset_file_path =
        (boost::filesystem::path(GtestsParameters::instance().dataset_dir) / "openeb" / "gen31_timer.raw").string();
    std::ifstream ifs(dataset_file_path);
    GenericHeader header(ifs);
...
TEST_F_WITH_DATASET(RAWEventFileWriter_Gtest, add_metadata_flush_raw_fail) {
...
TEST_F_WITH_DATASET(RAWEventFileWriter_Gtest, remove_metadata) {
    {
        std::string dataset_file_path =
            (boost::filesystem::path(GtestsParameters::instance().dataset_dir) / "openeb" / "gen31_timer.raw").string();
        std::ifstream ifs(dataset_file_path);
...
TEST_F_WITH_DATASET(RAWEventFileWriter_Gtest, add_metadata_map_from_camera) {
    std::string dataset_file_path =
        (boost::filesystem::path(GtestsParameters::instance().dataset_dir) / "openeb" / "gen31_timer.raw").string();
...
TEST_F_WITH_DATASET(RAWEventFileWriter_Gtest, write_raw) {
...
TEST_F(HDF5EventFileWriter_Gtest, constructor_invalid) {
    std::unique_ptr<HDF5EventFileWriter> writer;
    std::string file =
        (boost::filesystem::path(tmpdir_handler_->get_full_path("inexistent_directory")) / "file.hdf5").string();
...
TEST_F(HDF5EventFileWriter_Gtest, write_first_ts_is_big) {
    ...
    }
    {
        ...
        H5::H5File file(tmp_file_, H5F_ACC_RDONLY);

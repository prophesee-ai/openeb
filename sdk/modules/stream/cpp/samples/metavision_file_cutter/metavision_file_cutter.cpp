...
int main(int argc, char *argv[]) {
    ...
    if (out_file_ext == ".raw") {
        try {
            camera.raw_data();
            writer = std::make_unique<Metavision::RAWEventFileLogger>(out_file_path);
    ...
    if (out_file_ext == ".raw") {
        ...
        // Only makes sense when used with post decoder (i.e. not raw) stream as the decoder will apply a time-shift

...
// This application demonstrates how to use Metavision SDK Core and SDK Stream to convert
...
int convert_file_to_hdf5(const std::filesystem::path &in_file_path, const std::filesystem::path &out_hdf5_file_path) {
    ...
    hdf5_writer.add_metadata_map_from_camera(camera);

    // Process EventCD into HDF5 data
    try {
        camera.cd().add_callback(
            [&hdf5_writer](const Metavision::EventCD *begin, const Metavision::EventCD *end) {
                hdf5_writer.add_events(begin, end);
            });
    } catch (Metavision::CameraException &) {}

    // Process EventExtTrigger into HDF5 data
    try {
        camera.ext_trigger().add_callback(
            [&hdf5_writer](const Metavision::EventExtTrigger *begin, const Metavision::EventExtTrigger *end) {
                hdf5_writer.add_events(begin, end);
            });
    } catch (Metavision::CameraException &) {}
